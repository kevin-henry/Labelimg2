...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.findname = QLineEdit()
        self.findButton = QToolButton()
        ...
        self.findButton.setToolButtonStyle(Qt.ToolButtonIconOnly)
        self.delButton.setToolButtonStyle(Qt.ToolButtonIconOnly)
        ...
        self.controlButtonsLayout.addWidget(self.findname)
        self.controlButtonsLayout.addWidget(self.findButton)
        self.controlButtonsLayout.addWidget(self.delButton)
        ...
        delete_img = action('Delete\nImg', self.deleteSelectedImg,
                        '1', 'cancel2.svg', u'Delete', checkable=True, enabled=True)
        ...
        find = action('Find', self.findStart,
                    'Enter', 'play.svg', u'find img',
                    checkable=True, enabled=True)
        ...
        self.findButton.setDefaultAction(find)
        self.delButton.setDefaultAction(delete_img)
        ...
        if self.dirname:
            ...
            try:
                if ">" in name or "<" in name or "==" in name:
                    getList = self.fileModel.getStringList(imglist)
                    for per in getList:
                        num = per[1]
                        if num is None:
                            num = 0
                        expr = eval(str(num)+name)
                        if expr:
                            self.matchList.append(os.path.join(self.dirname,per[0]))
                else:
                    for img_name in imglist:
                        if re.findall(name.lower(),img_name.lower()):
                            self.matchList.append(img_name)
            except:
                pass
        if self.matchList:
            self.fileModel.setStringList(self.matchList)                      
    ...
    def deleteSelectedImg(self):
        img_path = self.filePath
        if os.path.exists(img_path):
            os.remove(self.filePath)
        if img_path in self.matchList:
            xml_path = img_path.rsplit('.',1)[0]+".xml"
            if os.path.exists(xml_path):
                os.remove(xml_path)
            Index = self.matchList.index(img_path)
            self.matchList.remove(img_path)
        self.fileModel.setStringList(self.matchList)
        if Index + 1 >= self.fileModel.rowCount():
            return False
        nextIndex = self.fileModel.index(Index)    
        self.filesm.setCurrentIndex(nextIndex, QItemSelectionModel.SelectCurrent)
...
def get_main_app(argv=[]):
    ...
    if not os.path.exists('data'):
        ...
        if not os.path.exists('data/predefined_classes.txt'):
            f = open('data/predefined_classes.txt','w')
            f.writelines('abc\n')
            f.close()
